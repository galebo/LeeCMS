...
public class UserFormController extends BaseFormController {
    ...
    public String onSubmit(User user, BindingResult errors, HttpServletRequest request,
        ...
        if (request.getParameter("delete") != null) {
        	commonService.getUserManager().removeUser(user.getId().toString());
    ...
    protected User showForm(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!isFormSubmission(request)) {
            ...
            if (userId == null && !isAdd(request)) {
                user = commonService.getUserManager().getUserByUsername(request.getRemoteUser());
            } else if (!StringUtils.isBlank(userId) && !"".equals(request.getParameter("version"))) {
                user = commonService.getUserManager().getUser(userId);
            } else {
                user = new User();
            ...
            return commonService.getUserManager().getUser(request.getParameter("id"));
        }
    }
