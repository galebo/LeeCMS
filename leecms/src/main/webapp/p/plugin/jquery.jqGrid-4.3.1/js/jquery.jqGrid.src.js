...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addJSONData = function(data,t, rcnt, more, adjust) {
			...
			ts.p.page = $.jgrid.getAccessor(data,dReader.page) || ts.p.page;
			lp = $.jgrid.getAccessor(data,dReader.total);
			ts.p.lastpage = lp === undefined ? ts.p.lastpage : lp;
			ts.p.records = $.jgrid.getAccessor(data,dReader.records) || ts.p.records;
		...
		updatepager = function(rn, dnd) {
			try {
				if (ts.p.treeGrid == true && ts.p.data[ts.p.data.length - 1].level != 0) {
					return;
				}
			}catch(e){
			...
			if(base < 0) { base = 0; }
			if(ts.p.treeGrid==true){
				base = base*parseInt(ts.p.rowTotal,10);
			}else{
